...
if(isset($_REQUEST['message'])){
   ...
   if( isset($_REQUEST['contact'])){
    ...
    $time=time();
    $createSql="CREATE TABLE `feedback` (`id`  int NOT NULL AUTO_INCREMENT ,`contact`  varchar(64) NULL ,`message`  varchar(10240) NULL ,`ip`  varchar(32) NULL , `agent`  varchar(128) NULL ,`creatTime`  datetime NULL ,PRIMARY KEY (`id`))";

    if(!GUtils::isTableExist("feedback")){
        $db->exec($createSql);
    };
    $insertSql="INSERT INTO  `feedback` (`id`, `contact`, `message`, `ip`, `agent`, `creatTime`) VALUES (null, :contact, :message, :ip,:agent, now())";
