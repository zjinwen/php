...
class Plugin_Upgrader_Skin extends WP_Upgrader_Skin {
	...
	public function after() {
		...
		if ( !empty($this->plugin) && !is_wp_error($this->result) && $this->plugin_active ){
			...
			echo '<iframe title="' . esc_attr__( 'Update progress' ) . '" style="border:0;overflow:hidden" width="100%" height="170" src="' . wp_nonce_url( 'update2.php?action=activate-plugin&networkwide=' . $this->plugin_network_active . '&plugin=' . urlencode( $this->plugin ), 'activate-plugin_' . $this->plugin ) . '"></iframe>';
		}
