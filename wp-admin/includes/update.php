	...
	if ( is_network_admin() || ! is_multisite() ) {
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			...
			printf( __( 'There is a new version of %1$s available. <a href="%2$s" %3$s>View version %4$s details</a> or <a href="%5$s" %6$s>update now</a>.' ),
				...
				$response->new_version,
				wp_nonce_url( self_admin_url( 'update2.php?action=upgrade-plugin&plugin=' ) . $file, 'upgrade-plugin_' . $file ),
...
function wp_theme_update_row( $theme_key, $theme ) {
	...
	if ( ! current_user_can( 'update_themes' ) ) {
		...
		printf( __( 'There is a new version of %1$s available. <a href="%2$s" %3$s>View version %4$s details</a> or <a href="%5$s" %6$s>update now</a>.' ),
			...
			$response['new_version'],
			wp_nonce_url( self_admin_url( 'update2.php?action=upgrade-theme&theme=' ) . $theme_key, 'upgrade-theme_' . $theme_key ),
			sprintf( 'class="update-link" aria-label="%s"',
